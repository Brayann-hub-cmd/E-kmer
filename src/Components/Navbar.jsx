...
export default function Navbar({ setTitle, setCategorie }) {
  ...
  // Composant avatar réutilisable sur les 3 layouts
  ...
  // ─────────────────────────────────────────────────────────
  return (
    <header className="bg-black dark:bg-gray-950 text-white sticky top-0 z-50 transition-colors">
        ...
        <div className="hidden lg:flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="relative" ref={languageDesktopRef}>
              ...
              {isLanguageOpen && (
                <div className="absolute top-full right-0 mt-2 bg-white text-gray-900 rounded-lg shadow-xl w-32 overflow-hidden z-50 border border-gray-100">
            ...
            {isLoggedIn ? (
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setShowLogoutModal(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 border border-gray-600 text-sm transition-colors">
              ...
              <div className="flex items-center gap-2">
                {/* Se connecter */}
                <Link to="auth/login"
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 border border-gray-600 text-sm whitespace-nowrap transition-colors">
        ...
        <div className="hidden md:flex lg:hidden items-center justify-between gap-2">
                  ...
                  <div className="absolute top-full left-0 mt-1 bg-white border rounded-lg shadow-xl w-48 max-h-80 overflow-y-auto z-50">
                    {categories.map((cat) => (
                      <button key={cat.code} onClick={() => handleCategoryChange(cat)}
                        className="w-full text-left px-3 py-2 text-xs hover:bg-orange-500 text-black hover:text-white">{cat.nom}</button>
                    ))}
                  </div>
                )}
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={languageTabletRef}>
              <LangBtn onClick={() => setIsLanguageOpen(!isLanguageOpen)} />
              {isLanguageOpen && (
                <div className="absolute top-full right-0 mt-2 bg-white text-gray-900 rounded-lg shadow-xl w-28 z-50 border border-gray-100">
            ...
            {isLoggedIn ? (
              <>
                <AvatarBtn size="sm" />
                <button onClick={() => setShowLogoutModal(true)}
                  className="p-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 border border-gray-600 transition-colors">
                  <FaSignOutAlt className="text-xs" />
                </button>
              </>
            ) : (
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={languageMobileRef}>
                <LangBtn onClick={() => setIsLanguageOpen(!isLanguageOpen)} />
                {isLanguageOpen && (
                  <div className="absolute top-full right-0 mt-2 bg-white text-gray-900 rounded-lg shadow-xl w-24 z-50 border border-gray-100">
              ...
              <div className="relative h-full" ref={categoryMobileRef}>
                ...
                {isCategoryOpen && (
                  <div className="absolute top-full left-0 mt-1 bg-white border rounded-lg shadow-xl w-48 max-h-60 overflow-y-auto z-50">
                    {categories.map((cat) => (
                      <button key={cat.code} onClick={() => handleCategoryChange(cat)}
                        className="w-full text-left px-3 py-2.5 text-xs hover:bg-orange-500 text-black hover:text-white">{cat.nom}</button>
                    ))}
                  </div>
                )}
              </div>

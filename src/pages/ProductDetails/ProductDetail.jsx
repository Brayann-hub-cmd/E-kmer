...
const ProductDetail = () => {
  ...
  const favoriteStorageKey = useMemo(() => `favorite_product_${id}`, [id]);
  ...
  useEffect(() => {
    if (!id) {
      setFetchError(t.noProductId || "Aucun identifiant de produit fourni");
      setLoading(false);
      return;
    }
    ...
    const loadProduct = async () => {
      ...
      try {
        const response = await api.get(`annonces/${id}/`, { signal: controller.signal });
        setProduct(response.data);
        setCurrentImageIndex(0);
        setQuantity(1);
      } catch (err) {
        ...
        if (err.response?.status === 404) {
          setFetchError(t.productNotFound || "Produit introuvable.");
        } else {
          setFetchError(t.productLoadError || "Impossible de charger le produit. Veuillez réessayer plus tard.");
        }
      } finally {
        setLoading(false);
      }
  ...
  const toggleFavorite = () => {
    ...
    setIsFavorite(next);
    localStorage.setItem(favoriteStorageKey, next ? 'true' : 'false');
  };

  const handleAddToCart = async () => {
    if (!product || !product.code || isOutOfStock) return;
    ...
    try {
      ...
      setActionMessage(t.addedToCart || "Produit ajouté au panier.");
    } catch (err) {
      console.error("Erreur ajout au panier:", err);
      setActionError(t.cartAddError || "Impossible d'ajouter au panier. Réessayez.");
    } finally {
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      console.error("Erreur achat immédiat:", err);
      setActionError(t.buyNowError || "Impossible de lancer l'achat. Réessayez.");
    } finally {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[65%_35%] gap-8">
          ...
          <div>
            <div className="sticky top-4 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 bg-white dark:bg-gray-800 transition-colors duration-300">
              ...
              {isOutOfStock && (
                <div className="mt-3 px-3 py-2 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-600 dark:text-red-300 text-sm">
                  <T>outOfStock</T>
                </div>
              )}
              ...
              <div className="flex flex-col gap-3 mt-4">
                <button
                  ...
                  className="flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 text-white font-medium rounded-xl py-3 px-4 transition-colors w-full"
                >
                  <FaShoppingCart className="text-base" />
                  {isAddingCart ? t.addingToCart || "Ajout..." : <T>addToCart</T>}
                </button>
                ...
                >
                  <FaPlusCircle className="text-base" />
                  {isBuyingNow ? t.processing || "Traitement..." : <T>buyNow</T>}
                </button>
              ...
              {actionMessage && (
                <p className="mt-4 text-sm text-green-600 dark:text-green-400">{actionMessage}</p>
              )}
              {actionError && (
